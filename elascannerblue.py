...
##############################
#here we change red to blue
#############################
			#if r>minR and g*ratR<r and b*ratR<r :
			if b>minR and g*ratR<b and r*ratR<b :
				scalepix.append((i,j))
				#pixels[i,j] = (0,0,255)
...
def single_LA():
	...
	if rCnt < 1:
		rCnt+=1
	leafarea = float(gCnt)/float(rCnt)*SSscale.get()
	if rCnt <2:
		rCnt = 0
	filelabel= Label (main, height =1, width=60)
	speedP=speedPscale.get()		
...
minR =200
minRscale = Scale(Frame3, from_=0, to=255, label="Scale minimum Blue RGB value:", orient=HORIZONTAL, tickinterval = 50, length = 250, variable = minR )
minRscale.set(20)
...
threshWscale.set(222)

ratR =1.3
ratRscale = Scale(Frame3, from_=1, to=2, resolution = 0.02, label="Scale Blue Ratio: (B/G & B/R)", orient=HORIZONTAL, tickinterval = 0.5, length = 200, variable = ratR )
ratRscale.set(1.14)
